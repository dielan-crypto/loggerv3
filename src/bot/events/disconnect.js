...
module.exports = {
  ...
  handle: () => {
    ...
    reconnects++
    global.logger.error(`Worker instance hosting ${cluster.worker.rangeForShard} on id ${cluster.worker.id} disconnected from the gateway. ${reconnects} out of 10.`) // eslint-disable-line no-undef
    if (reconnects >= 10) {
